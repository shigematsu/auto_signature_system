...
SQL
puts "drop table tcppackets_normal..."
...
SQL
puts "drop table udppackets_normal..."

# db.execute_batch(sql)

sql = <<SQL
CREATE TABLE tcppackets_normal (
    ...
    tcp_data      TEXT );
CREATE TABLE udppackets_normal (
    id            INTEGER  PRIMARY KEY AUTOINCREMENT,
    time          REAL NOT NULL,
    size          INTEGER NOT NULL,
...
File::open("/home/shigematsu/list/filelist_normal") {|file|
...
cap.loop do |pkt|
  if pkt.ip? and pkt.tcp?
    db.execute( " insert into tcppackets_normal values ( null, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", pkt.time.to_f, pkt.size, pkt.tcp_data_len, pkt.ip_src, pkt.tcp_sport, pkt.ip_dst, pkt.tcp_dport, pkt.tcp_flags_s, pkt.tcp_seq, pkt.tcp_data )
  end
  if pkt.ip? and pkt.udp?
    db.execute( " insert into udppackets_normal values ( null, ?, ?, ?, ?, ?, ?, ?, ?)", pkt.time.to_f, pkt.size, pkt.udp_len, pkt.ip_src, pkt.udp_sport, pkt.ip_dst, pkt.udp_dport, pkt.udp_data )
  end
end
cap.close
end
